...
class ufront_web_routing_Route extends ufront_web_routing_RouteBase {
	public function __construct($url, $handler, $defaults, $constraints) {
		...
		if(null === $url) {
			throw new HException(new thx_error_NullArgument("url", _hx_anonymous(array("fileName" => "Route.hx", "lineNumber" => 40, "className" => "ufront.web.routing.Route", "methodName" => "new"))));
		}
		...
		$this->url = $url;
		if(null === $handler) {
			throw new HException(new thx_error_NullArgument("handler", _hx_anonymous(array("fileName" => "Route.hx", "lineNumber" => 44, "className" => "ufront.web.routing.Route", "methodName" => "new"))));
		}
