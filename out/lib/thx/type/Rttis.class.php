...
class thx_type_Rttis {
	...
	static function unifyFields($cls, $h) {
		...
		if($parent !== null) {
			...
			default:{
				throw new HException(new thx_error_Error("Invalid type parent type ({0}) for class: {1}", new _hx_array(array($parent->path, $cls)), null, _hx_anonymous(array("fileName" => "Rttis.hx", "lineNumber" => 146, "className" => "thx.type.Rttis", "methodName" => "unifyFields"))));
			}break;
			}
		}
		return $h;
...
function thx_type_Rttis_2(&$cls, &$infos, &$name, &$x) {
	throw new HException(new thx_error_Error("not a class!", null, null, _hx_anonymous(array("fileName" => "Rttis.hx", "lineNumber" => 173, "className" => "thx.type.Rttis", "methodName" => "getClassDef"))));
}
