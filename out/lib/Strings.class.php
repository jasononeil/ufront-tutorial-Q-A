...
class Strings {
	...
	static function interpolatef($a, $b, $equation) {
		...
		return array(new _hx_lambda(array(&$a, &$b, &$equation, &$extract, &$fa, &$fb, &$functions, &$i, &$min, &$rest, &$sa, &$sb), "Strings_21"), 'execute');
	}
	static function ellipsis($s, $maxlen, $symbol) {
		...
		if(strlen($s) > $maxlen) {
			return _hx_substr($s, 0, Strings_22($maxlen, $s, $symbol)) . $symbol;
		} else {
			return $s;
		}
	}
	static function ellipsisf($maxlen, $symbol) {
		...
		return array(new _hx_lambda(array(&$maxlen, &$symbol), "Strings_23"), 'execute');
	}
...
function Strings_2(&$buf, &$culture, &$df, &$format, &$left, &$nullstring, &$p, &$params, &$pattern, &$pos, $f, $a1) {
	{
		return array(new _hx_lambda(array(&$a1, &$buf, &$culture, &$df, &$f, &$format, &$left, &$nullstring, &$p, &$params, &$pattern, &$pos), "Strings_24"), 'execute');
	}
}
...
function Strings_4(&$buf, &$culture, &$nullstring, &$pattern, $values) {
	{
		...
		return Iterators::map($buf->iterator(), array(new _hx_lambda(array(&$buf, &$culture, &$nullstring, &$pattern, &$values), "Strings_25"), 'execute'))->join("");
	}
}
...
function Strings_21(&$a, &$b, &$equation, &$extract, &$fa, &$fb, &$functions, &$i, &$min, &$rest, &$sa, &$sb, $t) {
	{
		return Iterators::map($functions->iterator(), array(new _hx_lambda(array(&$a, &$b, &$equation, &$extract, &$fa, &$fb, &$functions, &$i, &$min, &$rest, &$sa, &$sb, &$t), "Strings_26"), 'execute'))->join("");
	}
}
function Strings_22(&$maxlen, &$s, &$symbol) {
	{
...
function Strings_23(&$maxlen, &$symbol, $s) {
	{
		if(strlen($s) > $maxlen) {
			return _hx_substr($s, 0, Strings_27($maxlen, $s, $symbol)) . $symbol;
		} else {
			return $s;
		}
	}
}
function Strings_24(&$a1, &$buf, &$culture, &$df, &$f, &$format, &$left, &$nullstring, &$p, &$params, &$pattern, &$pos, $a2) {
	{
		return call_user_func_array($f, array($a1, $a2));
	}
}
function Strings_25(&$buf, &$culture, &$nullstring, &$pattern, &$values, $df, $_) {
	{
		return call_user_func_array($df, array($values));
	}
}
function Strings_26(&$a, &$b, &$equation, &$extract, &$fa, &$fb, &$functions, &$i, &$min, &$rest, &$sa, &$sb, &$t, $f, $_) {
	{
		return call_user_func_array($f, array($t));
	}
}
function Strings_27(&$maxlen, &$s, &$symbol) {
	{
		$a = strlen($symbol); $b = $maxlen - strlen($symbol);
		if($a > $b) {
			return $a;
