...
function haxe_rtti_XmlParser_0(&$l, $e1, $e2) {
	{
		$n1 = haxe_rtti_XmlParser_6($»this, $e1, $e2, $l);
		$n2 = haxe_rtti_XmlParser_7($»this, $e1, $e2, $l, $n1);
		if($n1 > $n2) {
			return 1;
		}
		return -1;
...
function haxe_rtti_XmlParser_2(&$»this, &$r) {
	switch($r) {
	case "null":{
...
function haxe_rtti_XmlParser_4(&$»this, &$c, &$x) {
	switch($c->getName()) {
	case "class":{
...
function haxe_rtti_XmlParser_5(&$»this, &$x) {
	switch($x->getName()) {
	case "unknown":{
...
function haxe_rtti_XmlParser_6(&$»this, &$e1, &$e2, &$l) {
	$»t = ($e1);
	switch($»t->index) {
	case 0:
	$p = $»t->params[0];
...
function haxe_rtti_XmlParser_7(&$»this, &$e1, &$e2, &$l, &$n1) {
	$»t = ($e2);
	switch($»t->index) {
	case 0:
	$p = $»t->params[0];
